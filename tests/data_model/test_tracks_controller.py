...
def test__add_nodes_no_seg(graph_2d):
    ...
    assert [1, node1] in tracks.graph.edge_attrs().select(
        ["source_id", "target_id"]
    ).to_numpy().tolist()
    assert [node1, node2] in tracks.graph.edge_attrs().select(
        ["source_id", "target_id"]
    ).to_numpy().tolist()
    ...
    # TODO: clean-up by defining graph.has_edge
    assert [3, node] in tracks.graph.edge_attrs().select(
        ["source_id", "target_id"]
    ).to_numpy().tolist()
    assert [node, 4] in tracks.graph.edge_attrs().select(
        ["source_id", "target_id"]
    ).to_numpy().tolist()
    assert [4, 5] not in tracks.graph.edge_attrs().select(
        ["source_id", "target_id"]
    ).to_numpy().tolist()

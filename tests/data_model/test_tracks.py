...
def test_create_tracks(graph_3d, segmentation_3d):
    ...
    tracks = Tracks(graph=graph_3d, segmentation_shape=(2, 100, 100, 100), ndim=4)
    ...
    tracks = Tracks(graph=graph_3d, segmentation_shape=(2, 100, 100, 100))
    assert tracks.get_positions([1]).tolist() == [[50, 50, 50]]
    assert tracks.get_time(1) == 0
    ...
    tracks_wrong_attr = Tracks(graph=graph_3d, segmentation_shape=(2, 100, 100, 100), pos_attr="test", ndim=4)
    ...
    tracks = Tracks(graph=graph_3d_copy, segmentation_shape=(2, 100, 100, 100), pos_attr=pos_attr, ndim=4)
    assert tracks.get_positions([1]).tolist() == [[50, 50, 50]]
...
def test_create_tracks_not_trackdata_graph():
    with pytest.raises(ValueError, match="graph must be a tracksdata.graph.GraphView"):
        Tracks(graph=None, segmentation_shape=(2, 100, 100, 100))


def test_pixels_and_seg_id(graph_3d):
    # create track with graph and seg
    tracks = Tracks(graph=graph_3d, segmentation_shape=(2, 100, 100, 100))
...
# def test_save_load_delete(tmp_path, graph_2d):
...
def test_nodes_edges(graph_2d):
    tracks = Tracks(graph_2d, segmentation_shape=(5, 100, 100), ndim=3)
...
def test_set_positions_list(graph_2d):
    node_ids = graph_2d.node_ids()
    positions = graph_2d.node_attrs()["pos"].to_numpy()

    graph_2d.add_node_attr_key("x", default_value=0.0)
    graph_2d.add_node_attr_key("y", default_value=0.0)

    graph_2d.update_node_attrs(
        attrs={"x": positions[:, 1], "y": positions[:, 0]},
        node_ids=node_ids,
    )

    tracks = Tracks(graph_2d, segmentation_shape=(5, 100, 100), pos_attr=["y", "x"], ndim=3)
    tracks.set_positions((1, 2), [(1, 2), (3, 4)])
    assert np.array_equal(
        tracks.get_positions((1, 2), incl_time=False), np.array([[1, 2], [3, 4]])
    )
...
def test_compute_node_attrs_from_pixels(graph_2d):
    tracks = Tracks(graph_2d, segmentation_shape=(5, 100, 100), ndim=3, scale=(1, 2, 2))
    attrs = compute_node_attrs_from_pixels(
        pixels=tracks.get_pixels([1, 2]), ndim=tracks.ndim, scale=tracks.scale
    )
    ...
    assert np.array_equal(attrs[NodeAttr.POS.value][0], np.array([100, 100]))
    assert np.array_equal(attrs[NodeAttr.POS.value][1], np.array([40, 160]))

    # cannot compute node attributes without segmentation
    # no longer tested, because graph always has masks, so area can be computed always
    # tracks = Tracks(graph_2d, segmentation_shape=(5, 100, 100), ndim=3)
    # attrs = compute_node_attrs_from_pixels(
    #     pixels=tracks.get_pixels([1, 2]), ndim=tracks.ndim, scale=tracks.scale
    # )
...
def test_compute_edge_attrs(graph_2d):
    tracks = Tracks(graph_2d, segmentation_shape=(5, 100, 100), ndim=3)
...
def test_get_pixels_and_set_pixels(graph_2d):
    tracks = Tracks(graph_2d, segmentation_shape=(5, 100, 100), ndim=3)
    pix = tracks.get_pixels([1])
    assert isinstance(pix, list)
    assert np.asarray(tracks.segmentation[0, 50, 50]) == 1
    tracks.set_pixels(pix, [99])

    # TODO: Teun: need cach.clear in set_pixels? (do asarray twice)
...
def test_set_pixels_none_value(graph_2d):
    tracks = Tracks(graph_2d, segmentation_shape=(5, 100, 100), ndim=3)
    pix = tracks.get_pixels([1])
    with pytest.raises(ValueError):
...
def test_compute_ndim_errors():
    g = create_empty_graphview_graph(database=":memory:", position_attrs=["pos"])
    ...
    #no longer necessary, because segmentation is always present
    # with pytest.raises(
    #     ValueError, match="Cannot compute dimensions from segmentation or scale"
    # ):
    #     Tracks(g, segmentation_shape=(1,1,1))

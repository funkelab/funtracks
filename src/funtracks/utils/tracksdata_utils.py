...
def create_empty_graphview_graph(
    ...
    if "node_solution" not in graph_td.node_attr_keys():
        graph_td.add_node_attr_key("node_solution", default_value=True, dtype=pl.Boolean)
    if "edge_solution" not in graph_td.edge_attr_keys():
        graph_td.add_edge_attr_key("edge_solution", default_value=True, dtype=pl.Boolean)

    graph_td_sub = graph_td.filter(
        td.NodeAttr("node_solution") == True,  # noqa: E712
        td.EdgeAttr("edge_solution") == True,  # noqa: E712
    ).subgraph()
...
def add_masks_and_bboxes_to_graph(
    ...
    graph._update_metadata(segmentation_shape=segmentation.shape)

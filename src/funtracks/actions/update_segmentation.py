...
class UpdateNodeSeg(BasicAction):
    ...
    def _apply(self) -> None:
        ...
        if value == 0:
            ...
            mask_old = self.tracks.graph_solution.nodes[self.node][self.mask_key]
            mask_subtracted = mask_old.__isub__(mask_new)
            self.tracks.update_mask(self.node, mask_subtracted, mask_key=self.mask_key)

        elif self.tracks.graph_solution.has_node(value):
            # if node already exists:
            mask_old = self.tracks.graph_solution.nodes[value][self.mask_key]

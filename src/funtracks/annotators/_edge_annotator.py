...
class EdgeAnnotator(GraphAnnotator):
    ...
    def compute(self, feature_keys: list[str] | None = None) -> None:
        ...
        if self.iou_key in keys_to_compute:
            ...
            for n in self.tracks.graph_solution.node_ids():
            ...
            for t in range(self.tracks.segmentation.shape[0] - 1):
                ...
                for node in nodes_in_t:
                    for succ in self.tracks.graph_solution.successors(node):
    ...
    def _iou_update(
        ...
        for edge in edges:
            source, target = edge
            mask1 = self.tracks.graph_solution.nodes[source]["mask"]
            mask2 = self.tracks.graph_solution.nodes[target]["mask"]
            iou = mask1.iou(mask2)
    ...
    def update(self, action: BasicAction):
        ...
        if isinstance(action, AddEdge):
            ...
            edges_to_update = []
            for pred in self.tracks.graph_solution.predecessors(modified_node):
                edges_to_update.append((pred, modified_node))
            for succ in self.tracks.graph_solution.successors(modified_node):

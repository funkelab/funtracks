...
class IoU(Feature):
    ...
    def update(self, project: Project, edge: tuple[int, int]):
        source, target = edge
        ...
        if len(ious) == 0:
            ...
            assert len(ious) == 1
            iou = ious[0]
        project.cand_graph.set_feature_value(edge, self, iou)
...
class Distance(Feature):
    ...
    def update(self, project: Project, edge: tuple[int, int]):
        ...
        project.cand_graph.set_feature_value(edge, self, dist)

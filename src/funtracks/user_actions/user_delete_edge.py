...
class UserDeleteEdge(ActionGroup):
    def __init__(
        ...
        super().__init__(project, actions=[DeleteEdge(project, edge)])
        if self.project.solution.has_edge(edge):
            ...
            if out_degree == 0:  # removed a normal (non division) edge
                ...
                sibling = next(self.project.solution.successors(edge[0]))
                new_track_id = self.project.solution.get_track_id(edge[0])
